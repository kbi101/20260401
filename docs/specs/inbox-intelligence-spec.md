...
public record EmailPayload(
    ...
    String gmailCategory   // Extracted from X-GM-LABELS (e.g., "Promotions", "Updates", "Primary")
) {}
...
public record EmailSummary(
    ...
    String sentiment,
    String timelordCategory, // AI-determined (JOB_SEARCH, FINANCE, GENERAL_ADS, OTHER)
...
public record EmailSummaryDetail(
    ...
    String gmailCategory,         // Gmail's native IMAP category
    String timelordCategory,      // AI's refined category
    String summaryText,
    List<String> keyActionItems,
    String sentiment,
    LocalDateTime processedAt
...
## 9. Two-Tier Classification Strategy
To support dynamic filtering and automated task generation, email payloads undergo a two-tier classification process:

### 9.1 Tier 1: Gmail Native Categories (gmailCategory)
- **Source:** Extracted directly during the IMAP synchronization phase from the Bronze layer.
- **Mechanism:** Parsed from the Gmail specific `X-GM-LABELS` extension (if connecting via standard IMAP) or mapped from specific IMAP folders (e.g., `[Gmail]/Promotions`, `[Gmail]/Updates`).
- **Standard Values:** `Primary`, `Promotions`, `Social`, `Updates`, `Forums`.
- **Purpose:** Used as a fast, deterministic filter before any AI processing occurs. 

### 9.2 Tier 2: Timelord Semantic Categories (timelordCategory)
- **Source:** Generated by the LLM during the Silver/Gold summarization phase.
- **Mechanism:** The prompt delivered to the `IntelligencePort` requires the LLM to categorize the context of the email based on its intent, overriding the native Gmail category which is often too broadly defined.
- **Standard Values:**
    - `JOB_SEARCH`: Interview requests, application confirmations, recruiter outreaches.
    - `FINANCE`: Bills due, banking statements, receipts, tax documents.
    - `GENERAL_ADS`: Marketing newsletters, promotional discounts (lowest priority).
    - `OTHER`: Personal correspondence or out-of-bounds content.
- **Purpose:** Used to feed specific Timelord subsystems (e.g., routing `FINANCE` emails to a budget tracking module, or treating `JOB_SEARCH` with high urgency).
